# these 2 modules are required. Neither should need to be installed, but they do need importing.
import random
import requests
...
def instructions():
    ...
    print("4. You then have the option to rerun with another date or to end the program")
...
def date_info():
    ...
    day = date[0:2]
    month = date[3:5]
    #variable needs to be global so it can be used outside the function
    global on_day
...
def choice():
    ...
    if option not in ["Event", "Birth", "Death"]:
        ...
        # using the length of the option list to form the range for random number generation.
        ...
        # I wanted to use ANSI escape code to change the year colour, but that doesn't work with writing to a .txt file.
        if option == "Event":
...
def run():
    # Function for running the On This Day output. Handy to have so the code down below is really simple!
    global my_date
    my_date = date_info()
    with open("on_this_day.txt", "a+") as otd:
        otd.write(on_day+"\n")
